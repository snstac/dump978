node(label: 'raspberrypi') {
    ...
    for (int i = 0; i < dist_arch_list.size(); ++i) {
        ...
        stage("Test install on ${dist} (${arch})") {
            sh "BRANCH=${env.BRANCH_NAME} ARCH=${arch} /build/pi-builder/scripts/validate-packages.sh ${dist} ${resultdir}/dump978-fa_*.deb ${resultdir}/skyaware978_*.deb ${resultdir}/skyview978_*.deb"
        }
    }
